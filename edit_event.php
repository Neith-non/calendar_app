...
// 1. Get the Event ID from URL
$publish_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
...
// Fetch all participants and group them
$partStmt = $pdo->query("
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($participant_ids)) {
        ...
        // --- SMART VENUE CHECKER (Excluding current event) ---
        if (!$is_off_campus) {
            $venueConflictStmt = $pdo->prepare("
                ...
                LIMIT 1
            ");
            $venueConflictStmt->execute([$venue_id, $publish_id, $end_datetime, $start_datetime]);
        ...
        // --- SMART PARTICIPANT CHECKER (Excluding current event) ---
        if (!$hasConflict) {
        ...
        if (!$hasConflict) {
            try {
                ...
                // Update Events Table
                $stmt_event = $pdo->prepare("UPDATE events SET category_id = ?, title = ?, description = ?, start_date = ?, start_time = ?, end_date = ?, end_time = ? WHERE publish_id = ?");
                $stmt_event->execute([$category_id, $title, $description, $start_date, $start_time, $end_date, $end_time, $publish_id]);

                // Clear old participants
                $stmt_del = $pdo->prepare("DELETE FROM participant_schedule WHERE event_publish_id = ?");
    ...
    <main class="flex-1 flex justify-center items-start overflow-y-auto p-4 sm:p-6 lg:p-10 relative custom-scrollbar">
        ...
        <div class="bg-white dark:bg-[#111827] rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm w-full max-w-4xl mx-auto overflow-hidden mt-4 sm:mt-0 flex flex-col h-auto">
            ...
            <div class="bg-slate-50 dark:bg-slate-900/50 p-6 sm:p-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0">
                <div>
                    <h2 class="text-2xl font-extrabold text-amber-600 dark:text-amber-500 flex items-center gap-3">
                    ...
                    <p class="text-slate-500 dark:text-slate-400 text-sm mt-2 font-medium ml-1">Modify details for this pending schedule.</p>
                </div>
                <a href="javascript:history.back()" class="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold py-2.5 px-4 rounded-xl transition-colors border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-2 text-sm shrink-0">
                ...
                <form action="edit_event.php?id=<?php echo $publish_id; ?>" method="POST" id="eventForm" class="space-y-12" x-data="{ selectedDept: 'all' }">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-4 mb-6">
                            <span class="bg-amber-500 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs font-black">1</span>
                            Event Details
                        </h3>
                        ...
                        <div class="space-y-5">
                            <div>
                                ...
                                <input type="text" name="title" required placeholder="e.g., Grade 10 Math Olympiad"
                                    value="<?php echo htmlspecialchars($_POST['title'] ?? $eventData['title']); ?>"
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="3" placeholder="Optional details, instructions, or agenda..."
                                    class="input-premium w-full px-4 py-3 rounded-lg font-medium text-sm resize-none"><?php echo htmlspecialchars($_POST['description'] ?? $eventData['description']); ?></textarea>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                ...
                                <div>
                                    ...
                                    <div x-data="{
                                            open: false,
                                            search: '',
                                            selectedId: '<?php echo $_POST['category_id'] ?? $eventData['category_id']; ?>',
                                            selectedName: '<?php 
                                                $catIdToMatch = $_POST['category_id'] ?? $eventData['category_id'];
                                                $catName = ''; 
                                                foreach($categories as $c){ 
                                                    if($c['category_id'] == $catIdToMatch) { $catName = $c['category_name']; break; }
                                                } 
                                            ...
                                            <ul class="max-h-48 overflow-y-auto custom-scrollbar py-1">
                                                <template x-for="opt in filteredOptions" :key="opt.id">
                                                    <li @click="selectOption(opt)" 
                                                        ...
                                                        :class="{'bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400': selectedId === opt.id}">
                                ...
                                <div>
                                    ...
                                    <div x-data="{
                                            open: false,
                                            search: '',
                                            selectedId: '<?php echo $_POST['venue_id'] ?? $eventData['venue_id']; ?>',
                                            selectedName: '<?php 
                                                $venIdToMatch = $_POST['venue_id'] ?? $eventData['venue_id'];
                                                $venueName = ''; 
                                                foreach($venues as $v){ 
                                                    if($v['venue_id'] == $venIdToMatch) { 
                    ...
                    <div>
                        <div class="border-b border-slate-100 dark:border-slate-800 pb-4 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <h3 class="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                <span class="bg-amber-500 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs font-black">2</span>
                                Schedule
                            </h3>
                            
                            <?php $is_all_day_check = ($eventData['start_time'] == '00:00:00' && $eventData['end_time'] == '23:59:59'); ?>
                            <label for="is_all_day" class="flex items-center gap-3 cursor-pointer bg-slate-50 dark:bg-slate-800 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-amber-500 transition-colors shadow-sm">
                                <span class="text-xs text-slate-600 dark:text-slate-300 font-bold uppercase tracking-wider">All-Day Event</span>
                                <div class="relative">
                                    <input type="checkbox" name="is_all_day" id="is_all_day" class="sr-only-custom peer" <?php echo (isset($_POST['is_all_day']) || $is_all_day_check) ? 'checked' : ''; ?>>
                                    <div class="w-10 h-5 bg-slate-300 dark:bg-slate-600 rounded-full peer peer-checked:bg-amber-500 transition-colors after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-transform peer-checked:after:translate-x-5"></div>
                                </div>
                            </label>
                        </div>
                                ...
                                <div class="space-y-4">
                                    <div>
                                        ...
                                        <input type="date" name="start_date" required id="main_start_date"
                                            value="<?php echo $_POST['start_date'] ?? $eventData['start_date']; ?>"
                                    ...
                                    <div class="main-time-input-container transition-all duration-300 overflow-hidden">
                                        ...
                                        <input type="time" name="start_time" id="main_start_time"
                                            value="<?php echo $_POST['start_time'] ?? $eventData['start_time']; ?>"
                                ...
                                <div class="space-y-4">
                                    <div>
                                        ...
                                        <input type="date" name="end_date" required id="main_end_date"
                                            value="<?php echo $_POST['end_date'] ?? $eventData['end_date']; ?>"
                                            class="input-premium w-full px-4 py-2.5 rounded-lg text-sm font-semibold cursor-pointer">
                                    </div>
                                    <div class="main-time-input-container transition-all duration-300 overflow-hidden">
                                        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Time</label>
                                        <input type="time" name="end_time" id="main_end_time" 
                                            value="<?php echo $_POST['end_time'] ?? $eventData['end_time']; ?>"
                                            class="input-premium w-full px-4 py-2.5 rounded-lg text-sm font-semibold cursor-pointer main-time-input">
                                    </div>
                                </div>
                            </div>
                    ...
                    <div>
                        <div class="border-b border-slate-100 dark:border-slate-800 pb-4 mb-6">
                            <h3 class="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                <span class="bg-amber-500 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs font-black">3</span>
                        ...
                        <div class="mb-6 bg-slate-50 dark:bg-slate-800/30 p-5 rounded-2xl border border-slate-100 dark:border-slate-800">
                            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2"><i class="fa-solid fa-sitemap text-amber-500 mr-2"></i>Select Department Category</label>
                                ...
                                <?php foreach ($grouped_participants as $dept => $parts): ?>
                                    ...
                                    <div x-show="selectedDept === 'all' || selectedDept === '<?php echo htmlspecialchars($dept, ENT_QUOTES); ?>'" 
                                        ...
                                        <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 mb-3 pb-2">
                                            <h4 class="text-[11px] font-extrabold text-amber-600 dark:text-amber-400 uppercase tracking-widest"><?php echo htmlspecialchars($dept); ?></h4>
                                            <label class="flex items-center space-x-1.5 cursor-pointer group">
                                                <input type="checkbox" data-target="dept-<?php echo $dept_id; ?>" class="select-all-dept w-4 h-4 rounded border-slate-300 dark:border-slate-600 text-amber-500 focus:ring-amber-500 bg-slate-50 dark:bg-slate-900 transition-colors cursor-pointer">
                                            ...
                                            <?php foreach ($parts as $p): ?>
                                                ...
                                                <label class="flex items-center space-x-3 cursor-pointer group p-1.5 rounded hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                                                    <input type="checkbox" name="participants[]" value="<?php echo $p['participant_id']; ?>" data-name="<?php echo $p['display_name']; ?>" <?php echo $isChecked; ?>
                                                        class="participant-cb w-4 h-4 rounded border-slate-300 dark:border-slate-600 text-amber-500 focus:ring-amber-500 bg-white dark:bg-slate-900 cursor-pointer">
                    ...
                    <div class="flex flex-col-reverse sm:flex-row gap-4 pt-6 border-t border-slate-100 dark:border-slate-800">
                        <a href="javascript:history.back()"
                            class="text-center bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold py-3.5 px-8 rounded-xl transition-colors border border-slate-200 dark:border-slate-700 shadow-sm text-sm">
                            Cancel
                        </a>
                        <button type="submit"
                            class="flex-1 bg-amber-500 hover:bg-amber-600 text-white font-bold py-3.5 rounded-xl transition-colors shadow-lg flex justify-center items-center gap-2 text-sm">
